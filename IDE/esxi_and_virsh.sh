...
esxi_count_disk_num(){
    ...
    vm_have_disk_num=0
    vm_have_disk_num=$(${host_ssh} "ls ${host_file_path}/${vm_name}*/*-flat.vmdk 2>/dev/null | wc -l")
    if [ $? -ne 0 ]
    then
        log error "get esxi vm ${vm_name} has disk num error"
        echo -1
        return
    fi
    echo ${vm_have_disk_num}
}
...
wget_vm_disk_file_to_local(){
    ...
    #delete, because this inspection is repettive
    #esxi_disk_num=0
...
local_vm_install_driver(){
    ...
    tar_nm=$(ls *.tar.gz | grep -i ${vm_type})
    ...
    tar -zxvf ${tar_nm} -C ${vm_tmp_path} &>/dev/null && cd ${vm_tmp_path} &>/dev/null
    if [ $? -ne 0 ]
    then
        log error "uncompress ${tar_nm} tar bag into ${vm_tmp_path} err"
        echo -1
        return
    fi
    ...
    #    echo "${vm_tmp_path##*/}" >> ${vm_tmp_path%/*}/migrate_log/too_many_disk.list
    #    echo -1 
    #    return
    #fi
    ...
    ./auto_exec.sh "${vm_version}" "${sys_file_nm}" "${nbd_num}" &>/dev/null
    if [ $? -ne 0 ]
    then
